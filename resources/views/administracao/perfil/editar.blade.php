...
<div class="profile-grid">
    <form class="admin-panel profile-card profile-edit-card" method="POST" action="{{ route('admin.perfil.atualizar') }}" enctype="multipart/form-data">
        ...
        <div class="profile-identity">
            ...
            <input id="photo" name="foto" type="file" accept="image/jpeg,image/png,image/webp" style="max-width:130px">
            <small>Até 2 MB.</small>
        </div>
        <div class="profile-fields">
            <label for="name">Nome completo</label>
            <input id="name" name="nome" required maxlength="100" value="{{ old('nome',auth()->user()->nomeAutoridade) }}">
            <label for="cpf">CPF</label>
            <input id="cpf" name="cpf" inputmode="numeric" maxlength="14" value="{{ old('cpf',auth()->user()->cpfAutoridade) }}">
            <label for="phone">Telefone</label>
            <input id="phone" name="telefone" type="tel" value="{{ old('telefone',$telefone->numTelefoneAutoridade ?? null) }}">
        </div>
