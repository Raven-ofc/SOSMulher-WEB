<aside class="admin-sidebar" aria-label="Menu principal">
    ...
    <nav class="admin-nav">
        ...
        <div class="registration-menu">
            <a href="{{ route('admin.vitimas.index') }}" class="admin-nav-item {{ in_array($active ?? '', ['admin.vitimas.index', 'admin.agressores.index', 'admin.devices', 'admin.medidas-protetivas']) ? 'is-active' : '' }}">
                @include('parciais.icone', ['name' => 'book'])
                Cadastros
            </a>
            @if(in_array($active ?? '', ['admin.vitimas.index', 'admin.agressores.index', 'admin.devices', 'admin.medidas-protetivas']))
