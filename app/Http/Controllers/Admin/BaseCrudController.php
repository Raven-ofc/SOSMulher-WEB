...
abstract class BaseCrudController extends \App\Http\Controllers\Controller
{
    ...
    public function destroy(int $id)
    {
        DB::transaction(function () use ($id) {
            ...
            $registro->delete();
            AdminAudit::record($this->tabelaAuditoria(), $id, 'remocao');
        });
    ...
    public function alterarStatus(Request $request, int $id)
    {
        ...
        DB::transaction(function () use ($id, $dados) {
            ...
            $registro->save();
            AdminAudit::record($this->tabelaAuditoria(), $id, $dados['status']);
        });
    ...
    protected function persistir(Request $request, ?int $id = null)
    {
        ...
        if (!$id) {
            $registro->{$this->colunaStatus()} = 'ativo';
        }
        $registro->save();
        $this->aposSalvar($registro, $dados, $id);
        AdminAudit::record($this->tabelaAuditoria(), $registro->id, $id ? 'atualizacao' : 'criacao');
    ...
    abstract protected function tabelaAuditoria(): string;
    ...
    protected function ordenacaoPadrao(): array { return ['id', 'asc']; }
    protected function dadosExtrasCriacao(): array { return []; }
    protected function dadosExtrasEdicao(): array { return []; }
    protected function dadosExtrasVisualizacao(Model $registro): array { return []; }
    protected function processarDados(Request $request, array $dados): array { return $dados; }
}